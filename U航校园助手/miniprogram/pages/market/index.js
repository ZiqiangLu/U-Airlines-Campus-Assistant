...
const { marketCategories, marketTypes } = require('../../utils/constants');
const { normalizePost } = require('../../utils/format');

function categoryName(key) {
  const item = marketCategories.find((category) => category.key === key);
  return item ? item.name : '物品交易';
}
...
Page({
  ...
  goPublish() {
    wx.navigateTo({ url: `/pages/publish/index?channel=market&type=${this.data.activeType}` });
  },
